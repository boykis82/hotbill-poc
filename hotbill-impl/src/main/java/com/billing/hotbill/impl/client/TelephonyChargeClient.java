 ...
 * 전화과금자료 시스템 클라이언트
 *
 * 이동전화 계약의 과금자료를 외부 시스템에 비동기로 요청하는 클라이언트
 * 실제 외부 API는 stub으로 처리
 */
@Component
public class TelephonyChargeClient implements ExternalChargeClient {

    private static final Logger log = LoggerFactory.getLogger(TelephonyChargeClient.class);

    private final RestTemplate restTemplate;
    private final String apiUrl;

    public TelephonyChargeClient(
            RestTemplate restTemplate,
            @Value("${external.telephony.api.url:http://localhost:8080/api/telephony/charge}") String apiUrl) {
    ...
    public void requestChargeDataAsync(List<String> contractNumbers, HotbillUsecase usecase) {
        log.info("전화과금자료 시스템 비동기 요청 시작 - 계약 수: {}, usecase: {}",
        ...
        try {
            ...
            log.info("전화과금자료 시스템 비동기 요청 완료 - 계약 수: {}", contractNumbers.size());

            // 비동기 요청이므로 즉시 반환
            // 외부 시스템은 나중에 콜백 API를 통해 과금자료를 전달함

        } catch (RestClientException e) {
            log.error("전화과금자료 시스템 호출 실패 - 계약 수: {}, usecase: {}, 오류: {}",
