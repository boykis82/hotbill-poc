...
/**
 * TelephonyChargeClient 테스트
 */
@ExtendWith(MockitoExtension.class)
class TelephonyChargeClientTest {

    @Mock
    private RestTemplate restTemplate;

    private TelephonyChargeClient telephonyChargeClient;

    private static final String TELEPHONY_API_URL = "http://localhost:8080/api/telephony/charge";

    @BeforeEach
    void setUp() {
        telephonyChargeClient = new TelephonyChargeClient(restTemplate, TELEPHONY_API_URL);
    }
    ...
    void requestChargeDataAsync_REST_API_호출_테스트() {
        ...
        // When
        telephonyChargeClient.requestChargeDataAsync(contractNumbers, usecase);

        // Then
        verify(restTemplate, times(1)).exchange(
            eq(TELEPHONY_API_URL),
            eq(HttpMethod.POST),
